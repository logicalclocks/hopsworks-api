...
class TestMonitoringWindowConfigEngine:
    ...
    def test_fetch_feature_group_data_with_model_filter(self, mocker, backend_fixtures):
        ...
        # Act — model monitoring always resolves event_time="log_time",
        ...
        # The event-time filter is half-open: >= start, < end, unlike the
    ...
    def test_fetch_feature_group_data_with_event_time_feature(
        ...
        # With an event_time_feature, the window is read as a plain
    ...
    def test_fetch_feature_view_data_with_event_time_feature(
        ...
        # With an event_time_feature, the FV query is filtered instead of
    ...
    def test_run_single_window_monitoring_cache_lookup_uses_event_time_bounds(
        ...
        """With event_time_feature set, the cache lookup uses event time.
    ...
    def test_run_single_window_monitoring_all_time_event_window_has_no_time_filter(
        ...
        """An ALL_TIME window on an event-time basis reads the latest snapshot.
    ...
    def test_run_single_window_monitoring_cache_lookup_uses_commit_bounds_without_event_time(
        ...
        the behaviour from before event-time windows.
        """
        fg = _make_hudi_fg("DELTA")
        window_config = mwc.MonitoringWindowConfig(
            window_config_type=mwc.WindowConfigType.ROLLING_TIME,
...
class TestMergeDispatch:
    ...
    def test_event_time_feature_set_skips_merge_path_even_when_eligible(self, mocker):
        """An event_time_feature skips the merge path even when eligible.
