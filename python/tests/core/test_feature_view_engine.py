...
class TestFeatureViewEngine:
    ...
    def test_get_training_data_time_range_in_read_options(self, mocker):
        # A time-range read converts the window to epoch millis and ships it
        # to the engine read as internal read options, pruning the increments
        # by their event-window partition key.
        # Arrange
        feature_store_id = 99
        ...
        mock_fv_engine_read_dir = mocker.patch(
    ...
    def test_get_training_data_time_range_in_memory_raises(self, mocker):
        ...
        assert "in-memory" in str(e_info.value)
    ...
    def test_recreate_training_dataset(self, mocker):
        ...
        # Act
        fv_engine._recreate_training_dataset(
            feature_view_obj=fv,
    ...
    def test_insert_training_data_time_series_split_raises(self, mocker):
        # Arrange: appending to a time-series split is refused — the batch
        # would land entirely in the last split and skew the dataset; the
        # supported pattern is an unsplit dataset read with a time range.
        ...
        # Act / Assert
        with pytest.raises(FeatureStoreException, match="time-series"):
            fv_engine._insert_training_data(
                feature_view_obj=fv,
                training_dataset_version=1,
                start_time="",
        ...
        mock_fv_engine_compute_training_dataset.assert_not_called()

    def test_insert_training_data_time_series_split_overwrite_allowed(self, mocker):
        # Arrange: overwrite rebuilds the splits over the full data, so it stays allowed.
        from hsfs.training_dataset_split import TrainingDatasetSplit
    ...
    def test_compute_training_dataset_td_spark_type(self, mocker):
        ...
        # Assert
        assert mock_fv_engine_get_training_dataset_metadata.call_count == 1
        assert mock_td_engine.return_value._read.call_count == 2
        assert mock_fv_engine_compute_training_dataset_statistics.call_count == 1
    ...
    def test_compute_training_dataset_statistics_default(self, mocker):
        ...
        # Act
        fv_engine._compute_training_dataset_statistics(
    ...
    def test_delete_training_dataset_only(self, mocker):
        ...
        # Act
        fv_engine._delete_training_dataset_only(
    ...
    def test_check_feature_group_accessibility(self, mocker):
        ...
        mocker.patch("hsfs.core.feature_view_api.FeatureViewApi")
        mock_engine_get_type = mocker.patch("hsfs.engine._get_type")
    ...
    def test_check_feature_group_accessibility_cache_feature_group_get_type_python(
        ...
        mock_engine_get_type = mocker.patch(
            "hsfs.engine._get_type", return_value="python"
        )
        mock_constructor_query = mocker.patch("hsfs.constructor.query.Query")
    ...
    def test_get_logging_feature_from_dataframe(self, mocker):
        ...
        # Act
        dataframe_logging_features = fv_engine._get_logging_feature_from_dataframe(
            fv, [df1, df2]
        )
    ...
    def test_get_training_dataset_schema_no_transformations(self, mocker):
        ...
        # Act
        schema = fv_engine._get_training_dataset_schema(fv)
    ...
    def test_get_training_dataset_schema_transformation_functions_statistics(
        ...
        # Act
        schema = fv_engine._get_training_dataset_schema(fv, 1)
