...
#
"""Spark withColumn-then-select regression tests.

These exercise the topological execution path in `Engine._apply_transformation_function`
...
def spark_engine():
    ...
    engine._set_instance(engine=eng, engine_type="spark")
    yield eng
...
class TestSparkDagRegression:
    ...
    def test_missing_feature_without_request_parameter_raises_typed_error(
        self, spark_engine
    ):
        """Missing-feature validation surfaces as TransformationFunctionException.

        When a UDF references a column that is neither in the data nor supplied
        via `request_parameters`, the engine's pre-flight validator must reject
        the call with a typed exception that names the missing feature — never
        let it slip through to Spark and surface as a raw `AnalysisException`.
        """
        df = pd.DataFrame({"col1": [1, 2, 3]})
        spark_df = spark_engine._spark_session.createDataFrame(df)
        ...
        with pytest.raises(TransformationFunctionException) as exc_info:
            TransformationFunctionEngine._apply_transformation_functions(
                execution_graph=dag, data=spark_df
            )
        assert "rp_value" in str(exc_info.value)
        assert exc_info.value.missing_features == {"rp_value"}
