...
class Engine:
    ...
    def validate_schema(self, feature_group, df, df_features):
        ...
        if not (is_pandas or is_polars):
            logging.info(
                "Input data is not a pandas or polars DataFrame. Skipping schema validation."
            )
            return df_features
        ...
        is_fg_created = bool(feature_group.id)

        # Check for null values in the primary key columns
        for pk in feature_group.primary_key:
            if (is_pandas and df[pk].isnull().any()) or (
                is_polars and df[pk].is_null().any()
            ):
        ...
        # Get string columns based on dataframe type
        string_columns = []
        if is_pandas:
    ...
    def validate_schema_old(self, feature_group, df, df_features):
        errors = {}
        # Check if the primary key columns exist
        for pk in feature_group.primary_key:
            if pk not in df.columns:
