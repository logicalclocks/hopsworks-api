...
class FeatureView:
        ...
        **kwargs,
        ...
        Parameters:
            ...
            lookback:
                ...
                For different windows per feature group, pass a `Lookback` — e.g. `Lookback(default=FeatureGroupLookback(...), feature_group_lookbacks={"dim_a": FeatureGroupLookback(...)})` or its dict form `{"default": {...}, "feature_group_lookbacks": {"dim_a": {...}}}`.
                See [`FeatureGroupLookback`][hsfs.constructor.lookback.FeatureGroupLookback] and [`Lookback`][hsfs.constructor.lookback.Lookback] for accepted key values, validation rules, and per-FG key matching semantics.
            partition_precision: Truncation of the event date keying the materialized partitions: `day`, `month` (the default) or `year`.
    ...
    def insert_training_data(
        ...
        The materialized data is stored in Hive partitions keyed by each row's UTC event date (a human-readable `YYYYMMDD` value), truncated to the training dataset's partition precision — `month` by default, or `day`/`year` for finer or coarser layouts, fixed when the version was created (see the `partition_precision` parameter of [`create_training_data`][hsfs.feature_view.FeatureView.create_training_data]).
    ...
    def get_training_data(
        ...
        **kwargs,
        ...
        The materialized data is stored in Hive partitions keyed by each row's UTC event date, truncated to the dataset's partition precision (month by default, or day/year), so a time-range read prunes to the matching partitions and never scans the rest of the data — for example, one growing time-series training dataset can serve `[t0, t1]` as the training set and `(t1, t2]` as the test set with two calls.
