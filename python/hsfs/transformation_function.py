...
class TransformationFunction:
    ...
    def _get_output_column_names(self) -> str:
        ...
        if self.transformation_type == TransformationType.MODEL_DEPENDENT:
            _BASE_COLUMN_NAME = (
                f'{self._hopsworks_udf.function_name}_{"_".join(self._hopsworks_udf.transformation_features)}_'
            )
            if len(self._hopsworks_udf.return_types) > 1:
                return [
                    f"{_BASE_COLUMN_NAME}{i}" for i in range(len(self._hopsworks_udf.return_types))
                ]
            else:
    ...
    @staticmethod
    def _validate_transformation_type(transformation_type : TransformationType, hopsworks_udf : HopsworksUdf):
        """
    ...
    def output_column_names(self) -> List[str]:
        ...
        if self._hopsworks_udf.function_name == "one_hot_encoder" and len(self._hopsworks_udf.output_column_names) != len(self._hopsworks_udf.return_types):
            self._hopsworks_udf.output_column_names = self._get_output_column_names()
