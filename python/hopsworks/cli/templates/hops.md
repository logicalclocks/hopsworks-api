---
description: Hopsworks software factories. `/hops ml` builds a verified, deployed ML system (feature, training and inference pipelines) recorded in system.yaml; `/hops dashboard` builds, edits or deletes a Superset dashboard over feature groups; `/hops app` builds, edits or deletes a Python app with a JavaScript UI. Also `/hops status`, `/hops verify`, `/hops stop`.
argument-hint: "[ml [reqs|data|features|train|infer|app] [--only] | dashboard | app | status | verify | stop] [instruction]"
---

You are running the `/hops` command with arguments: `$ARGUMENTS`
...
| First word | Do |
| --- | --- |
| `ml` | the ML system factory (below), from the phase named next, or resume |
| `reqs`, `data`, `features`, `train`, `infer` (and the old `f`, `t`, `i`) | the same as `ml <phase>` |
| `dashboard` | the dashboard factory, with the rest as the instruction |
| `app` | the app factory, with the rest as the instruction (the ML system's own app is `ml app`) |
| `status` | status (below); nothing else |
| `verify` | `verify` of the ML system here; `verify integration` or `verify benchmark` also runs those |
| `stop` | stop the ML system here |
| none | with a `system.yaml` in `<slug>/` under the working directory: resume it (`ml`); without one: ask which factory with `AskUserQuestion` (ML system, dashboard, app) |

Find a skill at `.claude/skills/<name>/` in the repository, else `~/.claude/skills/<name>/` (in a
Hopsworks terminal that links to `/opt/hops/agent-skills/`). Read its `SKILL.md` when a step
below names it; its `references/` are next to it.
...
Never reasoning, never waiting on the build. With a `system.yaml` here, run the system's own
renderer and print its output verbatim:

```bash
python <slug>/status.py            # add --cluster to include what the cluster says
```

From a second terminal, `python <slug>/status.py --watch 30` answers while a phase runs.
Without a `system.yaml`, list the project's apps (`hops app list`, with state and serving) and
dashboards (`hops superset dashboard list` in a terminal, marking those with a program under
`dashboards/`; from an external client, the programs under `./dashboards/` only, since listing
Superset from outside the cluster takes a job).

---

## /hops ml: the ML system factory
...
template, bundles, tests, the repository contract).
...
- **Run onwards.** `/hops ml <phase>` runs that phase and then every later phase that is not
...
### Before reqs: the repository and the system directory

Follow `hops-reqs/references/repo.md`: `gh auth status` must pass (otherwise say how to fix it and
stop); ask whether to use the current GitHub repository or create one with `gh repo create`; cut
`hops/<slug>` from the default branch. Propose the slug from the problem (lowercase, hyphens) and
create the system directory from the template:

```bash
python .claude/skills/hops-reqs/references/new_system.py <repo>/<slug>    # or ~/.claude/skills/...
```

Record `system.repo`. Every phase ends with one commit, `[<slug>] <phase>: <one line>`, pushed.

### reqs

The conversation in `hops-reqs/SKILL.md` ("The requirements conversation"), through
`AskUserQuestion`, two or three questions per call. Run `hops context`, `hops fg list`,
`hops datasource list` and `hops search <term>` first and show candidates by name. For each
data source settle its route and what the data phase needs (`references/data-sources.md`): for a
new connector, list the options its type requires and record them under `needs`, never asking
for a secret. Ask the sizing tier once, `small` proposed. A task outside classification,
regression and forecasting, or an agent system, is captured in full and the command stops after
`reqs` saying v1 builds none of it. Print the requirements back and ask to proceed; write
`requirements.status: met` on yes. That is the last stop until an escalation, the app question or
the pull request.
...
deployment, the entity, the consumers) and run the matching factory below with its program or
source inside the system: `<slug>/dashboards/<slug>.py` or `<slug>/app/`. Record `app`. On no,
`app: {wanted: false, status: skipped}`.
...
## /hops dashboard: a Superset dashboard over feature groups

Load **hops-superset**. Programs live one per dashboard at `Users/<user>/dashboards/<slug>.py`:
`~/dashboards/` in a Hopsworks terminal; from a laptop `./dashboards/`, mirrored with
`hops files upload` / `hops files download`. Each is `hops-superset/references/dashboard_program.py`
filled in. Superset is reachable only inside the cluster: in a terminal, run a program and the
`hops superset` commands directly; from an external client, run the program as a job instead
(`hops job deploy <slug>-dashboard dashboards/<slug>.py --env python-feature-pipeline
--upload-dir Users/<user>/dashboards --overwrite --run --wait`, then `hops job logs <slug>-dashboard
--stdout`), and list with the same job and `--args "--list"`. First check Superset answers
(`hops superset dataset list` in a terminal; the `--list` job externally); if not, say so and stop.

1. List the dashboards. With dashboards, ask: edit one (listed by title, marked when a
   program exists under `dashboards/`), create a new one, or delete one. With none, create.
2. **Edit.** The user picks one and says what should change. With a program: change it, update its
   docstring, run it, print the URL. Without one (built by hand in the UI): first write a program
   that reproduces it from `hops superset dashboard info` and each chart's `hops superset chart
   info`, say so, then apply the change, so the dashboard has a program from then on.
3. **Create.** `hops fg list`, filtered to feature groups materialized offline (an online-only
   group is listed as unavailable, since Superset reads through Trino), shown with grain and event
   time, multi-select. Read the columns (`hops fg features <name>`) and propose a few named charts
   that fit them: a big-number total, a bar over the categorical column with the fewest distinct
   values, a histogram of a numeric column, a time series on the event time when there is one. The
   user keeps, drops or adds in words. Propose a title from the table names. Write the program,
   run it, print the URL, confirm by listing again.
4. **Delete.** Confirm the exact title. With a program: run it with `--delete`, then remove the
   program file (and its HopsFS copy). Without one: list its charts and datasets first, then
   `hops superset dashboard delete <id> --yes` (in a terminal).

A chart Superset rejects is fixed in the program and the program rerun, at most three times per
chart; a chart that still fails is dropped with a line saying why.

## /hops app: a Python app with a JavaScript UI

Load **hops-app**. `hops app list` first; an instruction that names an existing app (or, with
several, the one the user picks) is an edit, "delete" is a delete, anything else is a new app.

- **Create.** From the description (what it shows, what a person can do, what it reads) write
  the app from `hops-app/references/app_skeleton/`: FastAPI backend with `/health` and a JSON API
  under `/api`, static ES-module UI with relative URLs, the description as the module docstring.
  Source at `Users/<user>/apps/<name>/` (terminal `~/apps/<name>/`; laptop `./apps/<name>/`
  uploaded with `hops files upload`), or git-backed when the working directory is a GitHub
  repository and the user prefers it. Environment by the reuse rule (`python-agent-pipeline` has
  FastAPI; otherwise a clone `<name>-app-env`). Then `hops app create <name> --path
  /Projects/<project>/Users/<user>/apps/<name>/app.py --app-kind custom --entrypoint-command
  "python app.py" --app-port 8080 --readiness-probe-path /health --environment <env> --start`,
  wait for RUNNING and serving (the readiness probe on `/health` has passed), and in a Hopsworks
  terminal smoke-test the pod through `kubectl port-forward`: the page and one call of each API
  route the description implies (the proxy URL needs a browser session). Print the URL.
- **Fix loop.** On a failed start, a stop in serving, or a failed smoke test: `hops app logs
  <name>`, name the cause in one line, change the source, upload, then `hops app redeploy <name>`
  for a running app or `hops app start <name>` for one that failed, and smoke-test again; at
  most five attempts, then stop with the last lines of the logs and what was tried. A missing library is a pin in the clone, never a workaround. Nothing in the loop asks.
- **Edit.** Change the source as asked, update the docstring, redeploy, rerun the smoke tests.
- **Delete.** Confirm the exact name, `hops app delete <name> --yes`, remove the source directory
  and, when one was created for this app, its environment clone.
