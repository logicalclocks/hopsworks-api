...
class JobsApi:
    ...
    def setup_flink_cluster(self, name, config=None):
        if self.exists(name):
            ...
            if config is None:
                config = self.get_configuration("FLINK")
                config["appName"] = name
            return self.create_job(name, config)

    def get_jobs_from_flink_cluster(self, execution):
        _client = client.get_instance()
    ...
    def stop_flink_cluster(self, execution):
        _client = client.get_instance()
    ...
    def stop_flink_cluster_job(self, execution, flink_cluster_job):
        _client = client.get_instance()
        path_params = [
            "hopsworks-api",
            "flinkmaster",
    ...
    def get_jars_from_flink_cluser(self, execution):
        _client = client.get_instance()
    ...
    def upload_jar_to_flink_cluster(self, execution, jar_file_path):
        _client = client.get_instance()
        path_params = [
            "hopsworks-api",
            "flinkmaster",
    ...
    def submit_job_to_flink_cluster(
    ...
    def flink_job_status(self, execution, flink_cluster_job):
        _client = client.get_instance()
        path_params = [
            "hopsworks-api",
            "flinkmaster",
