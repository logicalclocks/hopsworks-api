...
class DatasetApi:
    ...
    def list(self, path: str, offset: int = 0, limit: int = 1000) -> List[str]:
        ...
        # Returns
            `list[str]`: List of files and directories in the provided path
        # Raises
            `hopsworks.client.exceptions.RestAPIError`: If the backend encounters an error when handling the request
        """
        _client = client.get_instance()
        ...
        is_root_path = os.path.dirname(normalized_path) == "/" or "/" not in normalized_path
        if is_root_path:
            normalized_path = "" if normalized_path == "/" else normalized_path
            cls = dataset.Dataset
        else:
            cls = inode.Inode
    ...
    @usage.method_logger
    def _list_dataset_path(self, path: str, cls: Union[Type[dataset.Dataset], Type[inode.Inode]], offset: int = 0, limit: int = 1000):
        """
