...
if [[ ${ghprbPullTitle} =~ (FSTORE-[0-9]+) || ${ghprbPullTitle} =~ (HWORKS-[0-9]+) ]]; then
  ...
  labels=$(echo "${loadtest_prs}" | jq -r --arg captured_string ${captured_string} '.[] | select(.title | contains($captured_string)) | .labels[] | select(.name | contains("e2e")) | map(.name) | join(",")')
fi
...
if [ -z "${loadtest_branch}" ]; then
  ...
  if [ -z "${minikube_ip}" ]; then
    ...
    echo "Found minikube ip labels: ${minikube_ip}"
  fi
