---
description: Hopsworks ML system factory. Builds a verified, deployed ML system (feature, training and inference pipelines) recorded in system.yaml, from the requirements conversation to a pull request. Phases can be run alone; also verify and stop.
...
| First word | Do |
| --- | --- |
| none | with a `system.yaml` above: resume it; without one: start with the repository and `reqs` |
...
`/hops status` prints the phase table on a fast model; menus, dashboards and apps are `/hops`.
...
`AskUserQuestion`, two or three questions per call. The feature groups and data sources listed
at the top are current; run `hops search <term>` only when the user names something not there,
and show candidates by name. For each
