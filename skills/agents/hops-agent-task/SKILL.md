...
user, token, host="github.com")`), so a PAT stored for git also signs copilot in, unless it is a classic
`ghp_` token, which is skipped so it cannot shadow a terminal login. A login made in the terminal (`claude`, `codex`, or
...
| `environmentName` | `agent-job` (default) or a clone of it; anything else is rejected with 400 |
...
Tasks run in the `agent-job` environment: the Claude Code and Codex CLIs plus
the Python feature-engineering libraries. To add libraries, clone `agent-job`
...
- Extra libraries in the pod: **hops-environments** (clone `agent-job`).
